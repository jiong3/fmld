...
#![allow(clippy::all, clippy::pedantic, clippy::nursery, reason = "one-off functions, mostly generated by LLMs")]
...
pub fn add_sentences_from_json(
    conn: &Transaction,
    json_path: &str,
    with_ascii_tags: Vec<char>,
) -> Result<(), Box<dyn Error>> {
    ...
    let file = File::open(json_path)?;
    let reader = BufReader::new(file);
    // Parse list of tuples: (dst_trad, dst_simp, dst_ext_def_id,[(word_trad_or_simp, word_simp, word_ext_def_id)], translation)
    let entries: Vec<(String, String, usize, Vec<(String, Option<String>, Option<usize>)>, String)> =
        serde_json::from_reader(reader)?;

    let mut stmt_insert_shared = conn.prepare_cached(
        "INSERT INTO dict_shared (rank, rank_relative) VALUES (?1, ?2)",
    )?;
    ...
    let mut stmt_insert_sentence = conn.prepare_cached(
        "INSERT INTO dict_sentence (shared_id, ext_sent_id, for_word_id, for_definition_id, translation) VALUES (?1, ?2, ?3, ?4, ?5)",
    )?;

    let mut stmt_insert_sent_word = conn.prepare_cached(
        "INSERT INTO dict_sentence_word (sentence_id, word_rank, word_id, definition_id, ascii_txt) VALUES (?1, ?2, ?3, ?4, ?5)",
    )?;

    for (dst_trad, dst_simp, dst_ext_def_id, sentence_words, sent_en) in entries {
        // Resolve target word
        let dst_word_id = match db_read::get_word_id(conn, &dst_trad, &dst_simp)? {
            Some(id) => id,
            None => {
                eprintln!("Sentence target word not found: {} {}", dst_trad, dst_simp);
                continue;
            }
        };

        // Resolve target definition
        let dst_def_id = match db_read::get_definition_id_for_ext_id(conn, dst_word_id, dst_ext_def_id)? {
            Some(id) => id,
            None => {
                eprintln!("Sentence target definition not found: {} {} #{}", dst_trad, dst_simp, dst_ext_def_id);
                continue;
            }
        };

        let mut words_to_insert = Vec::new();
        let mut skip_sentence = false;

        // Process words for the sentence
        for (w_trad_or_simp, w_simp, w_ext_def_id) in &sentence_words {
            let ext_def_id_u32 = w_ext_def_id.map(|id| id as u32);
            let matches = db_read::get_word_def_ids(conn, w_trad_or_simp, w_simp.as_deref(), ext_def_id_u32);
            
            if matches.is_empty() {
                // Not found as a word (e.g. punctuation, spaces, numbers, or English). Put in ascii_txt.
                words_to_insert.push((None, None, Some(w_trad_or_simp.clone())));
            } else if matches.len() > 1 {
                // Ambiguous match, skip sentence and log it
                let sent_str = sentence_words.iter().map(|(w, _, _)| w.as_str()).collect::<Vec<_>>().join("");
                eprintln!("Ambiguous word '{}' in sentence: {}", w_trad_or_simp, sent_str);
                skip_sentence = true;
                break;
            } else {
                let (w_id, d_id) = matches[0];
                words_to_insert.push((Some(w_id), d_id, None));
            }
        }
        ...
        // Get rank of the definition
        let def_rank: i64 = conn.query_row(
            "SELECT s.rank FROM dict_definition d JOIN dict_shared s ON d.shared_id = s.id WHERE d.id = ?1",
            params![dst_def_id],
            |row| row.get(0),
        )?;

        // Find the highest rank_relative for this definition
        let max_rank_rel: Option<i64> = conn.query_row(
            "SELECT MAX(sh.rank_relative) FROM dict_sentence s JOIN dict_shared sh ON s.shared_id = sh.id WHERE s.for_definition_id = ?1",
            params![dst_def_id],
            |row| row.get(0),
        )?;
        let next_rank_rel = max_rank_rel.map_or(1, |r| r + 1);

        // Find the highest ext_sent_id for this word to ensure uniqueness
        let max_ext_sent_id: Option<u32> = conn.query_row(
            "SELECT MAX(ext_sent_id) FROM dict_sentence WHERE for_word_id = ?1",
            params![dst_word_id],
            |row| row.get(0),
        )?;
        let next_ext_sent_id = max_ext_sent_id.unwrap_or(0) + 1;

        // 1. Insert shared entry
        stmt_insert_shared.execute(params![def_rank, next_rank_rel])?;
        let shared_id = conn.last_insert_rowid();

        // 2. Attach tags
        for &tag_id in &tag_ids {
            stmt_add_tag.execute(params![shared_id, tag_id])?;
        }

        // 3. Insert sentence record
        stmt_insert_sentence.execute(params![
            shared_id,
            next_ext_sent_id,
            dst_word_id,
            dst_def_id,
        ...
        let sentence_id = conn.last_insert_rowid();

        // 4. Insert each parsed word
        for (i, (w_id, d_id, ascii_txt)) in words_to_insert.into_iter().enumerate() {
            let word_rank = i + 1;
            stmt_insert_sent_word.execute(params![
                sentence_id,
                word_rank,
                w_id,
                d_id,
