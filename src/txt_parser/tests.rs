...
// ParserIterator tests
#[test]
fn test_iterator_simple() {
    let lines = vec!["W| |詞".to_string(), " P| |ci2".to_string()];
    let mut parser = ParserIterator::new(lines.into_iter());
    ...
    assert_eq!(
        ...
        Some(ParsedLine {
            ...
            indentation: 0,
            line: "W| |詞".to_string(),
            parsed_line: Ok(DictLine::Word(vec![WordTagGroup {
                tags: vec![],
                words: vec![Word {
                    trad: "詞".to_string(),
                    simp: None,
    ...
    assert_eq!(
        ...
        Some(ParsedLine {
            ...
            source_line_num: 1,
            indentation: 1,
            line: "P| |ci2".to_string(),
            parsed_line: Ok(DictLine::Pinyin(vec![PinyinTagGroup {
                tags: vec![],
                pinyins: vec!["ci2".to_string()],
            }])),
        })
    );
...
#[test]
fn test_iterator_line_continuation() {
    let lines = vec![
        "N1 This is a long note".to_string(),
        "  that continues on the next line.".to_string(),
        "# A comment".to_string(),
    ];
    let mut parser = ParserIterator::new(lines.into_iter());
    ...
    assert_eq!(
        ...
        Some(ParsedLine {
            ...
            source_line_num: 2,
            indentation: 0,
            line: "N1 This is a long note\nthat continues on the next line.".to_string(),
            parsed_line: Ok(DictLine::Note(Note {
                id: 1,
                is_link: false,
    ...
    assert_eq!(
        ...
        Some(ParsedLine {
            ...
            source_line_num: 1,
            indentation: 0,
            line: "# A comment".to_string(),
            parsed_line: Ok(DictLine::Comment("A comment".to_string())),
        })
    );
    assert_eq!(parser.next(), None);
}
...
#[test]
fn test_iterator_complex_structure() {
    let lines = vec![
        "W| |單詞".to_string(),
        " P| |dan1ci2".to_string(),
        "  C n.".to_string(),
        "   D1| |a word".to_string(),
        " X=|s|詞語".to_string(),
        "# end of entry".to_string(),
    ];
    let mut parser = ParserIterator::new(lines.into_iter());

    // W line
    let next = parser.next().unwrap();
    assert_eq!(next.indentation, 0);
    assert_eq!(next.source_line_start, 1);
    assert_eq!(
        next.parsed_line,
        Ok(DictLine::Word(vec![WordTagGroup {
            tags: vec![],
    ...
    // P line
    let next = parser.next().unwrap();
    assert_eq!(next.indentation, 1);
    assert_eq!(next.source_line_start, 2);
    assert_eq!(
        next.parsed_line,
        Ok(DictLine::Pinyin(vec![PinyinTagGroup {
            tags: vec![],
    ...
    // C line
    let next = parser.next().unwrap();
    assert_eq!(next.indentation, 2);
    assert_eq!(next.source_line_start, 3);
    assert_eq!(next.parsed_line, Ok(DictLine::Class("n.".to_string())));

    // D line
    let next = parser.next().unwrap();
    assert_eq!(next.indentation, 3);
    assert_eq!(next.source_line_start, 4);
    assert_eq!(
        next.parsed_line,
        Ok(DictLine::Definition(DefinitionTag {
            id: 1,
    ...
    let next = parser.next().unwrap();
    assert_eq!(next.indentation, 1);
    assert_eq!(next.source_line_start, 5);
    assert!(matches!(next.parsed_line, Ok(DictLine::CrossReference(_))));

    // # line
    let next = parser.next().unwrap();
    assert_eq!(next.indentation, 0);
    assert_eq!(next.source_line_start, 6);
    assert_eq!(
        next.parsed_line,
        Ok(DictLine::Comment("end of entry".to_string()))
    );
...
#[test]
fn test_iterator_handles_empty_lines() {
    let lines = vec![
        "W| |word".to_string(),
        "".to_string(),
        " P| |pinyin".to_string(),
    ...
    let next = parser.next().unwrap();
    assert_eq!(next.indentation, 0);
    assert!(matches!(next.parsed_line, Ok(DictLine::Word(_))));

    let next = parser.next().unwrap();
    assert_eq!(next.indentation, 1);
    assert!(matches!(next.parsed_line, Ok(DictLine::Pinyin(_))));
